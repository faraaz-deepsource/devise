...
Rails.application.routes.draw do
  ...
  devise_for :user, only: [:registrations], controllers: { registrations: 'custom/registrations' }, as: :custom,
                    path: :custom
  ...
  authenticate(:admin, ->(admin) { admin.active? }) do
  ...
  authenticated :admin, ->(admin) { admin.active? } do
  ...
  devise_for :headquarters_admin, class_name: 'Admin', path: 'headquarters',
  ...
  devise_for :htmlonly_admin, class_name: 'Admin', skip: %i[confirmations unlocks], path: 'htmlonly_admin',
                              format: false, skip_helpers: %i[confirmations unlocks]
  devise_for :htmlonly_users, class_name: 'User', only: %i[confirmations unlocks], path: 'htmlonly_users',
                              format: false, skip_helpers: true
  ...
  scope ':locale', module: :invalid do
    devise_for :accounts, singular: 'manager', class_name: 'Admin',
                          ...
                          }, failure_app: lambda { |_env|
                                            [404, { 'Content-Type' => 'text/plain' }, ['Oops, not found']]
                                          }, module: :devise
  end
  ...
  namespace :sign_out_via, module: 'devise' do
    ...
    devise_for :delete_or_posts, sign_out_via: %i[delete post], class_name: 'Admin'
  end
