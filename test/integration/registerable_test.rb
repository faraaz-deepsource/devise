...
class RegistrationTest < Devise::IntegrationTest
  test 'a guest admin should be able to sign in successfully' do
    ...
    admin = Admin.to_adapter.find_first(order: %i[id desc])
  ...
  test 'a guest user should be able to sign up successfully and be blocked by confirmation' do
    ...
    user = User.to_adapter.find_first(order: %i[id desc])
  ...
  test 'a guest user cannot sign up with invalid information' do
    ...
    if defined?(Mongoid) && Mongoid::VERSION.to_f < 2.1
      (pending 'Fails on Mongoid < 2.1'
       break)
    end
  ...
  test 'an admin sign up with valid information in JSON format should return valid response' do
    post admin_registration_path(format: 'json'),
         params: { admin: { email: 'new_user@test.com', password: 'new_user123',
  ...
  test 'a user sign up with valid information in JSON format should return valid response' do
    post user_registration_path(format: 'json'),
         params: { user: { email: 'new_user@test.com', password: 'new_user123', password_confirmation: 'new_user123' } }
    assert_response :success
  ...
  test 'a user sign up with invalid information in JSON format should return invalid response' do
    post user_registration_path(format: 'json'),
         params: { user: { email: 'new_user@test.com', password: 'new_user123', password_confirmation: 'invalid' } }
  ...
  test 'a user update information with valid data in JSON format should return valid response' do
    ...
    put user_registration_path(format: 'json'),
        params: { user: { current_password: '12345678', email: 'user.new@test.com' } }
    assert_response :success
  ...
  test 'a user update information with invalid data in JSON format should return invalid response' do
    user = sign_in_as_user
    put user_registration_path(format: 'json'),
        params: { user: { current_password: 'invalid', email: 'user.new@test.com' } }
    assert_response :unprocessable_entity
