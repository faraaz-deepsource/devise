...
class AuthenticationRedirectTest < Devise::IntegrationTest
  ...
  test 'require_no_authentication should set the already_authenticated flash message as admin' do
    store_translations :en,
                       devise: { failure: { admin: { already_authenticated: 'You are already signed in as admin.' } } } do
      sign_in_as_admin
...
class AuthenticationSessionTest < Devise::IntegrationTest
  ...
  test 'refreshes _csrf_token' do
    swap ApplicationController, allow_forgery_protection: true do
      ...
      token_from_env = request.env['action_controller.csrf_token'] if Devise::Test.rails71_and_up?

      sign_in_as_user
      assert_not_equal request.session[:_csrf_token], token_from_session

      assert_not_equal request.env['action_controller.csrf_token'], token_from_env if Devise::Test.rails71_and_up?
    end
  end
