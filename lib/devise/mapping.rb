...
module Devise
  ...
  class Mapping # :nodoc:
    ...
    def default_failure_app(options)
      ...
      return unless @failure_app.is_a?(String)

      ref = Devise.ref(@failure_app)
      @failure_app = ->(env) { ref.get.call(env) }
    end
    ...
    def default_used_route(options)
      singularizer = ->(s) { s.to_s.singularize.to_sym }

      @used_routes = if options.has_key?(:only)
                       routes & Array(options[:only]).map(&singularizer)
                     elsif options[:skip] == :all
                       []
                     else
                       routes - Array(options[:skip]).map(&singularizer)
                     end
    end
    ...
    def default_used_helpers(options)
      ...
      @used_helpers = if options[:skip_helpers] == true
                        ...
                        routes - Array(skip).map(&singularizer)
                      else
                        routes
                      end
    end
